...
from uhtk.UTIL.colorful import print亮绿
...
class Net(nn.Module):
    ...
    def _act(self, obs=None, test_mode=None, eval_mode=False, eval_actions=None, avail_act=None):
        ...
        # at_bac = my_view(baec,[0,-1])
        logits = self.policy_head(at_bac)
        
        # apply action selector
        act, actLogProbs, distEntropy, probs = self._logit2act(logits, 
                                                               eval_mode=eval_mode,
                                                               test_mode=test_mode, 
